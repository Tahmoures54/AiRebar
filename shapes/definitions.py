...
Key improvement:
- get_shape_keys_by_standard(code) returns ONLY that standard's shapes.
  Never falls back to all shapes for an unknown code (prevents UI pollution).
"""
...
logger = logging.getLogger("RebarAgent.Shapes")
...
_BS_DRAW_MAP = {
    "00": "draw_straight", "11": "draw_l_bar", "12": "draw_z_bar",
    "13": "draw_double_cranked_bar", "14": "draw_double_cranked_bar",
    "15": "draw_multi_leg_chair", "16": "draw_multi_leg_chair",
    "21": "draw_closed_stirrup_90", "22": "draw_u_bar_with_hooks",
    "23": "draw_u_bar_with_hooks", "24": "draw_u_bar_with_hooks",
    "25": "draw_closed_stirrup_90", "26": "draw_multi_leg_stirrup",
    "27": "draw_closed_stirrup_90", "28": "draw_multi_leg_stirrup",
    "31": "draw_helical", "32": "draw_helical", "33": "draw_helical",
    "34": "draw_helical", "35": "draw_helical", "36": "draw_helical",
    "41": "draw_chair", "42": "draw_chair", "43": "draw_chair",
    "44": "draw_chair", "45": "draw_chair", "46": "draw_chair",
    "47": "draw_chair", "48": "draw_chair",
    "51": "draw_closed_stirrup_90", "52": "draw_closed_stirrup_90",
    "53": "draw_closed_stirrup_90", "54": "draw_multi_leg_stirrup",
    "55": "draw_multi_leg_stirrup", "56": "draw_helical", "57": "draw_helical",
    "58": "draw_helical", "59": "draw_helical",
    "61": "draw_chair", "62": "draw_chair", "63": "draw_spacer_bar",
    "64": "draw_spacer_bar", "65": "draw_multi_leg_chair",
    "66": "draw_u_bar_with_hooks", "67": "draw_multi_leg_stirrup",
    "68": "draw_multi_leg_chair",
    "70": "draw_closed_stirrup_90", "71": "draw_closed_stirrup_90",
    "72": "draw_u_bar", "73": "draw_u_bar_with_hooks", "74": "draw_u_bar",
    "75": "draw_multi_leg_stirrup", "76": "draw_multi_leg_stirrup",
    "77": "draw_circular_tie", "78": "draw_circular_tie", "79": "draw_circular_tie",
    "81": "draw_l_bar", "82": "draw_straight_with_90_hook", "83": "draw_z_bar",
    "84": "draw_double_hook_90", "85": "draw_l_bar_with_hook",
    "86": "draw_s_bar", "87": "draw_s_bar", "88": "draw_s_bar", "89": "draw_s_bar",
    "90": "draw_u_bar", "91": "draw_l_bar", "92": "draw_z_bar", "93": "draw_s_bar",
    "94": "draw_u_bar", "95": "draw_multi_leg_stirrup", "96": "draw_z_bar",
    "97": "draw_z_bar", "98": "draw_u_bar", "99": "draw_custom",
}
...
class ShapeRegistry:
        ...
        with self._lock:
            ...
            for cat_name, shapes in (json_cats or {}).items():
                ...
                for k, v in shapes.items():
                    cat_shapes[k] = self._normalize_shape_def(k, v, standard_code="json")
            ...
            # Custom shapes only under "custom" — do NOT mix into every standard
            if custom_cat:
                self._shapes_by_standard_code["custom"] = sorted(custom_cat.keys())
            else:
                self._shapes_by_standard_code["custom"] = []

            for key, data in flat.items():
                if not isinstance(data, dict):
                    continue
                sc = str(data.get("standard_code") or "").strip().lower()
                if not sc or sc == "json":
                    continue
                bucket = self._shapes_by_standard_code.setdefault(sc, [])
                if key not in bucket:
                    bucket.append(key)
            for sc in list(self._shapes_by_standard_code.keys()):
                self._shapes_by_standard_code[sc] = sorted(set(self._shapes_by_standard_code[sc]))
    ...
    def get_shape_keys_by_standard(self, code: str) -> list:
        """Return shapes for a standard code only. Unknown/empty → []."""
        code = (code or "").strip().lower()
        if not code:
            return []
        if code in self._shapes_by_standard_code:
            return list(self._shapes_by_standard_code.get(code, []))
        return []

    def get_shape_def(self, key: str) -> dict:
        if not key:
            return None
        shape = self.flat_shapes.get(key)
        if shape:
            return shape
        key_s = str(key).strip()
        prefix = key_s + " - "
        for k, v in self.flat_shapes.items():
            if k.startswith(prefix) or k.split(" - ")[0] == key_s:
                return v
        for k, v in self.flat_shapes.items():
            if isinstance(v, dict) and str(v.get("code", "")) == key_s:
                return v
        return None
    ...
    def list_standards(self) -> list:
        ...
        for code, display in STANDARD_DISPLAY_MAP.items():
            out.append((code, display, len(self.get_shape_keys_by_standard(code))))
        return out

    def health_report(self) -> dict:
        report = {
            "total_shapes": len(self.flat_shapes),
            "by_standard": {},
            "missing_calc_length": [],
            "missing_draw_func": [],
            "missing_params": [],
        ...
        for code in list(STANDARD_DISPLAY_MAP.keys()):
            keys = self.get_shape_keys_by_standard(code)
            report["by_standard"][code] = len(keys)
            if code != "custom" and len(keys) == 0:
                report["empty_standards"].append(code)
        for key, data in self.flat_shapes.items():
            if not isinstance(data, dict):
                continue
            if not callable(data.get("calc_length")):
                report["missing_calc_length"].append(key)
            if not data.get("draw_func"):
                report["missing_draw_func"].append(key)
            if not data.get("params"):
                report["missing_params"].append(key)
        return report

    def validate_shape_key(self, key: str) -> tuple:
        shape = self.get_shape_def(key)
        if not shape:
            return False, f"Unknown shape: {key}"
        if not callable(shape.get("calc_length")):
            return False, f"Shape '{key}' has no length calculator"
        if not shape.get("params"):
            return False, f"Shape '{key}' has no parameters"
        if not shape.get("draw_func"):
            return False, f"Shape '{key}' has no draw function"
        return True, "ok"
    ...
    def _eval_formula_safe(self, formula_str, params, dia):
        ...
        local_vars = {
            "p": params, "dia": dia, "math": math, "abs": abs,
